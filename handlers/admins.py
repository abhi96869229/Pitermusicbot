...
async def pause(client: Client, message: Message):
    ...
    await message.reply_text("✯Piter✯=⏸ Paused.")
...
async def resume(client: Client, message: Message):
    ...
    await message.reply_text("✯Piter✯=▶️ Resumed.")
...
async def stop(client: Client, message: Message):
    ...
    await message.reply_text("✯Piter✯=⏹ Stopped streaming.")
...
async def skip(client: Client, message: Message):
    ...
    await message.reply_text("✯Piter✯=🔄 Processing")
    if sira.is_empty(chat_id):
        tgcalls.pytgcalls.leave_group_call(chat_id)
        await message.reply_text("✯Piter✯=❗ Nothing in queue")
    else:
        tgcalls.pytgcalls.change_stream(
            chat_id, sira.get(chat_id)["file_path"]
        )

        await message.reply_text("✯Piter✯=⏩ Skipped the current song.")
...
async def admincache(client, message: Message):
    ...
    await message.reply_text("✯Piter✯=❇️ Admin cache refreshed!")
